...
spec_result_clear_result <- list(
  ...
  #' both `dbSendQuery()`
  clear_result_return_query = function(ctx) with_connection({
      res <- dbSendQuery(con, trivial_query())
      expect_invisible_true(dbClearResult(res))
  }), # with_connection

  #' and `dbSendStatement()`.
  clear_result_return_statement = function(ctx) with_connection({
  ...
  cannot_clear_result_twice_query = function(ctx) with_connection({
      res <- dbSendQuery(con, trivial_query())
      dbClearResult(res)
