...
spec_sql_quote_literal <- list(
  ...
  quote_literal_roundtrip = function(ctx, con) {
    ...
    do_test_literal <- function(x) {
      ...
      literals <- purrr::map_chr(x, dbQuoteLiteral, conn = con)
      ...
      is_logical <- purrr::map_lgl(x, is.logical)
      x_out[is_logical] <- purrr::map(x_out[is_logical], as.logical)
      is_numeric <- purrr::map_lgl(x, is.numeric)
      x_out[is_numeric] <- purrr::map(x_out[is_numeric], as.numeric)
