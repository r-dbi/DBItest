...
#' @format NULL
#' @keywords NULL
spec_arrow_get_stream <- list(
  arrow_get_stream_formals = function() {
    # <establish formals of described functions>
    expect_equal(names(formals(dbGetStream)), c("conn", "statement", "..."))
  },

  arrow_get_stream_atomic = function(con) {
    #' @return
    #' `dbGetStream()` always returns an object coercible to a [data.frame]
    #' with as many rows as records were fetched and as many
    #' columns as fields in the result set,
    #' even if the result is a single value
    query <- trivial_query()

    rows <- check_stream(dbGetStream(con, query))
    expect_equal(rows, data.frame(a = 1.5))
  },

  arrow_get_stream_one_row = function(con) {
    #' or has one
  ...
  #'
  arrow_get_stream_closed_connection = function(ctx, closed_con) {
    #' @section Failure modes:
    #' An error is raised when issuing a query over a closed
    expect_error(dbGetStream(closed_con, trivial_query()))
  },

  arrow_get_stream_invalid_connection = function(ctx, invalid_con) {
    #' or invalid connection,
    expect_error(dbGetStream(invalid_con, trivial_query()))
  },

  arrow_get_stream_syntax_error = function(con) {
    #' if the syntax of the query is invalid,
    expect_error(dbGetStream(con, "SELLECT"))
  },

  arrow_get_stream_non_string = function(con) {
    #' or if the query is not a non-`NA` string.
    expect_error(dbGetStream(con, character()))
    expect_error(dbGetStream(con, letters))
    expect_error(dbGetStream(con, NA_character_))
  },

  arrow_get_stream_record_batch_reader = function(ctx, con) {
    #' The object returned by `dbGetStream()` can also be passed to
  ...
  #  #' The following arguments are not part of the `dbGetStream()` generic
  ...
  #
  #  #'
  #  arrow_get_stream_params = function(ctx, con) {
  ...
  #      ret <- dbGetStream(con, query, params = params)
  #      expect_equal(ret, trivial_df(3), info = placeholder)
  #    }
  #  },
  #
  #  arrow_get_stream_immediate = function(con, table_name) {
  ...
  #    res <- expect_visible(dbGetStream(con, trivial_query(), immediate = TRUE))
