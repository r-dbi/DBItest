...
- a connector that describes how to establish the database connection, see ``?DBI::`DBIConnection-class` `` for details,
- tweaks, see `?tweaks`,
...
One way to scout for the reason of the problem is to review the sources of DBItest and relate the test name (that is printed with each failure) with the human-readable specification embedded with the test code.
